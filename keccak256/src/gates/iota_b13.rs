...
pub struct IotaB13Config<F> {
    ...
    // Contains `is_mixing` flag at Rotation::next() and ROUND_CTANT_B13 at Rotation::cur()
...
impl<F: FieldExt> IotaB13Config<F> {
    ...
    pub fn configure(
        ...
        meta.create_gate("iota_b13 gate", |meta| {
            // We do a trick which consists on multiplying an internal selector which is
            // always active by the actual `is_mixing` flag which will then enable or disable the gate.
            let q_enable = {
                // We query the flag value from the`round_ctant_b13` `Advice` column at rotation next
                // and multiply to it the active selector so that we avoid the `PoisonedConstraints` and each
                // gate equation can be satisfied while enforcing the correct gate logic.
                let flag = Expression::Constant(F::one())
                    - meta.query_advice(round_ctant_b13, Rotation::next());
                // Note also that we want to enable the gate when `is_mixing` is false. (flag = 0).
                // Therefore, we need to do `1-flag` in order to enforce this.
    ...
    /// Assigns the ROUND_CONSTANTS_BASE_13 to the `absolute_row` passed as an absolute instance column.
    ...
    fn test_iota_b13_gate() {
        ...
        impl<F: FieldExt> Circuit<F> for MyCircuit<F> {
            ...
            fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
                ...
                // Allocate space for the round constants in base-13 which is an instance column
        ...
        {
            // With the correct input and output witnesses, the proof should pass.
            ...
            // With wrong input and/or output witnesses, the proof should fail to be verified.
            let circuit = MyCircuit::<pallas::Base> {
                in_state,
                out_state: in_state,
                round_ctant: PERMUTATION - 1,
        ...
        // With flag set to `true`, the gate shouldn't trigger. And so we can pass any witness data and the proof should pass.
        {
            let circuit = MyCircuit::<pallas::Base> {
                in_state,
                out_state: in_state,
